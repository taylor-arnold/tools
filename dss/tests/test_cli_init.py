...
class TestInitCommand:
    ...
    def test_init_creates_manifest(self, working_directory: Path, cli_runner: CliRunner, mock_uuid):
        """Test that init creates a manifest file."""
        result = cli_runner.invoke(main, ['init'])
        
        assert result.exit_code == 0
        assert "Initialized empty manifest: manifest.yml (UUID: test-uuid-1234)" in result.output
    ...
    def test_init_creates_gitignore(self, working_directory: Path, cli_runner: CliRunner, mock_uuid):
    ...
    def test_init_does_not_overwrite_gitignore(self, working_directory: Path, cli_runner: CliRunner, mock_uuid):
    ...
    def test_init_fails_if_manifest_exists(self, working_directory: Path, cli_runner: CliRunner):
    ...
    def test_init_loads_remote_config(self, mock_home, working_directory: Path, cli_runner: CliRunner, mock_uuid, temp_dir: Path):
    ...
    @patch('dss.cli.Path.home')
    def test_init_handles_missing_remote_config(self, mock_home, working_directory: Path, cli_runner: CliRunner, mock_uuid, temp_dir: Path):
    ...
    @patch('dss.cli.Path.home')
    def test_init_handles_invalid_remote_config(self, mock_home, working_directory: Path, cli_runner: CliRunner, mock_uuid, temp_dir: Path):
