...
class TestFullWorkflow:
    ...
    def test_complete_local_workflow(self, working_directory: Path, cli_runner: CliRunner, mock_uuid):
    ...
    def test_complete_remote_workflow(self, mock_subprocess, working_directory: Path, cli_runner: CliRunner, mock_uuid):
        ...
        assert manifest_data["datasets"]["dataset1.txt"]["remote@1"] == "test-uuid-1234/dataset1.txt"
        assert manifest_data["datasets"]["dataset2.txt"]["remote@1"] == "test-uuid-1234/dataset2.txt"
        ...
        ssh_calls = [call for call in mock_subprocess.call_args_list if 'ssh' in str(call)]
        rsync_calls = [call for call in mock_subprocess.call_args_list if 'rsync' in str(call)]
    ...
    def test_backward_compatibility_workflow(self, working_directory: Path, cli_runner: CliRunner, mock_uuid):
    ...
    def test_error_handling_workflow(self, working_directory: Path, cli_runner: CliRunner):
    ...
    def test_mixed_file_operations(self, working_directory: Path, cli_runner: CliRunner, mock_uuid):
        ...
        result = cli_runner.invoke(main, ['add', 'valid1.txt', 'test_dir', '.hidden', 'nonexistent.txt', 'valid2.txt'])
